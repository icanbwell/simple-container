...
WORKDIR /usr/src/simple_container/
...
COPY Pipfile* /usr/src/simple_container/
...
# RUN apk add --no-cache curl libstdc++ libffi

# Install pipenv to manage and run the application
RUN pip install --no-cache-dir pipenv

# Set environment variables for project configuration
ENV PROJECT_DIR=/usr/src/simple_container
ENV FLASK_APP=simple_container.api
...
COPY ./simple_container ${PROJECT_DIR}

# Copy the Pipfile.lock from the previous stage in case it was locked
COPY --from=python_packages /usr/src/simple_container/Pipfile.lock /usr/src/simple_container/Pipfile.lock

# Copy Pipfile.lock to a temporary directory so it can be retrieved if needed
COPY --from=python_packages /usr/src/simple_container/Pipfile.lock /tmp/Pipfile.lock
